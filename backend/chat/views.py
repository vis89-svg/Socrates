...
class SharePageView(APIView):
    ...
    def get(self, request, token):
        ...
        html = (
            ...
            '.bubble{max-width:80%;padding:0.7rem 1rem;border-radius:14px;background:#fff;white-space:pre-wrap;word-break:break-word;}'
            '.msg.user .bubble{background:#1877f2;color:#fff;}'
            ...
            f'<div>{rows}</div>'
            '<p style="color:#888;font-size:0.8rem">Shared via Socrates</p>'
            '</div></body></html>'
        )
